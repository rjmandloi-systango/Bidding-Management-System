...
//show product list of current user Logged in
        ...
        if (typeof (snapshot) !== 'undefined') {
            ...
            if (snapshot.exists()) {
                snapshot.forEach((child) => {
                    ...
                    <td style="text-align:center"><button onclick="removeProduct(${userDATA.id} ,${child.val().ProductId})"><svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" fill="currentColor" class="bi bi-trash" viewBox="0 0 16 16">
...
//remove Product from currently logged in user's DB 
window.removeProduct = function (userId, productId) {
    remove(ref(db, `User/${userId}/Details/ProductSold/${productId}`), {
    }).then(() => {
        // remove the product from bidding also so as to avoid inconsistency
