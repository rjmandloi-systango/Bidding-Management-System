...
if (currentMonth < 10) {
  //formating the date in yyyy-mm-dd format
  ...
  get(child(databaseRef, "User/")).then((snapshot) => {
    if (typeof snapshot !== "undefined") {
      ...
      productDeatils.forEach((element) => {
        // console.log('line no 43:',element?.id);

        if (element?.id) {
          Object.keys(element.id).forEach((key) => {
            // console.log(key,'all present products id');
            createCard();
            //create card for all the products
            // console.log(element.id[key].ProductName,'take all product data similarly');
            function createCard() {
              ...
              let productState;
              // console.log(ProductStatus,'if product state are sold not undefine');
              if (ProductStatus) {
              ...
              // console.log(productState,'all product data one by one....');
              //card content
              let productContent = `
              ...
              <div class="card-body  rounded-3" >
                 ...
                 <div class="d-flex justify-content-between" >
                  ...
                  <details>
                    ...
                    <p class="card-text text-start  text-dark"><span class ="fw-bold"></span>${capitalize(
                      productDiscription
                    )}</p>
                    </details>
                  </div>
...
async function timer(
  ...
  let currentProductId =
    document.getElementById(uniqueProductId).dataset.productId;
  let productState =
    document.getElementById(currentProductId).dataset.productState;
  if (productState == "expired") {
    ...
    let timeFunction = setInterval(async function () {
      ...
      let days = Math.floor(
        distanceBetweenBidEndTimeAndCurrentTime / (1000 * 60 * 60 * 24)
      );
      ...
      let minutes = Math.floor(
        (distanceBetweenBidEndTimeAndCurrentTime % (1000 * 60 * 60)) /
          (1000 * 60)
      );
      let seconds = Math.floor(
        (distanceBetweenBidEndTimeAndCurrentTime % (1000 * 60)) / 1000
      );
      ...
      await get(child(databaseRef, `Winners/${currentProductId}`)).then(
        (snapshot) => {
          if (typeof snapshot !== "undefined") {
            if (snapshot.exists()) {
      ...
      let currentUserId =
        document.getElementById(uniqueProductId).dataset.productOwnerId;
      let currentBuyerId =
        document.getElementById(currentProductId).dataset.maxBidderId;
      ...
      if (distanceBetweenBidEndTimeAndCurrentTime <= 0) {
        ...
        await get(
          child(
            databaseRef,
            `User/${currentUserId}/Details/ProductSold/${currentProductId}/`
          )
        ).then((snapshot) => {
          if (typeof snapshot !== "undefined") {
            if (snapshot.exists()) {
              if (snapshot.val().ProductStatus) {
                // if product status exists and is set to sold
        ...
        let productStatus;
        if (
          document.getElementById(`maxBidderName_${currentProductId}`)
            .innerHTML == "--::--"
        ) {
        ...
        if (isAlreadySold != true) {
          ...
          if (currentBuyerId != "") {
            await update(
              ref(
                db,
                `User/${currentBuyerId}/Details/PurchasedProducts/` +
                  [currentProductId]
              ),
              {
...
function fetchHighestBidder(highestBidder) {
  ...
  productIds.forEach((key) => {
    ...
    document.getElementById(
      `mb_${highestBidder[key].ProductID}`
    ).innerHTML = `${highestBidder[key].BuyerBidMoney}`;
    let person = userDeatils.find(
      (user) => user.id === `${highestBidder[key].BuyerID}`
    );
    document.getElementById(
      `maxBidderName_${highestBidder[key].ProductID}`
    ).innerText = capitalize(`${person.FirstName}`);
    document.getElementById(key).dataset.maxBidderId =
      highestBidder[key].BuyerID; //new added
  });
}
  ...
  if (Object.keys(highestBidder).length !== 0) {
    set(ref(db, "Winners" + "/" + [expiredProductId] + "/"), {
      ...
      .then(() => {
        ...
        fetchEmails(
          highestBidder[expiredProductId].BuyerBidMoney,
          highestBidder[expiredProductId].BuyerID,
          highestBidder[expiredProductId].ProductID,
          highestBidder[expiredProductId].SellerID
        );
      })
      .catch((error) => {});
  }
...
async function sendEmail(
  ...
  if (productData != undefined) {
    ...
    document.getElementById("productInformationWinner").value = `
              ...
              `;
    document
      .getElementById("Winner-Form")
      .addEventListener("submit", function (event) {
        event.preventDefault();
        emailjs.sendForm("service_t4pf0da", "template_l9m8yup", this).then(
          async function () {},
          function (error) {
    ...
    document
      ...
      .addEventListener("submit", function (event) {
        ...
        emailjs
          .sendForm("service_t4pf0da", "template_l9m8yup", this)
